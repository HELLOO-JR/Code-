...
local function BuildUI()
    ...
    CreateToggle(aimbotContent, "FOV Circle", "Bật/tắt hiển thị vòng tròn phạm vi ngắm", {"Aimbot", "FOVCircle"})
    CreateSlider(aimbotContent, "Max Distance", "Khoảng cách tối đa để tự động ngắm (m)", {"Aimbot", "MaxDistance"}, 10, 450, 5)
    ...
    AddSection(aimbotContent, "TỐC ĐỘ BẮN")
    CreateToggle(aimbotContent, "Fast Reload", "Thay đạn nhanh hơn", {"Aimbot", "FastReload"})
    CreateToggle(aimbotContent, "Rapid Fire", "Bắn liên tục tốc độ cao", {"Aimbot", "RapidFire"})
    CreateSlider(aimbotContent, "Fire Delay", "Tốc độ bắn (giây)", {"Aimbot", "FireDelay"}, 0.01, 1.0, 0.01)
    CreateSlider(aimbotContent, "Reload Delay", "Tốc độ thay đạn (giây)", {"Aimbot", "ReloadDelay"}, 0.01, 1.0, 0.01)

    AddSection(aimbotContent, "KHÓA MỤC TIÊU")
    CreateToggle(aimbotContent, "Aim Lock", "Khóa mục tiêu đang ngắm, không đổi mục tiêu đột ngột", {"Aimbot", "AimLock"})
    CreateToggle(aimbotContent, "Auto Reload Cancel", "Hủy thay đạn khi có địch trong tầm ngắm", {"Aimbot", "AutoReloadCancel"})
    CreateToggle(aimbotContent, "Crosshair Target", "Hiển thị dấu chéo lên mục tiêu đang ngắm", {"Aimbot", "CrosshairTarget"})
    ...
    AddSection(espContent, "NÂNG CAO ESP")
    CreateToggle(espContent, "Hitbox Viewer", "Hiển thị hitbox các bộ phận cơ thể", {"ESP", "HitboxViewer"})
    CreateToggle(espContent, "Distance Marker", "Hiển thị cột mốc khoảng cách", {"ESP", "DistanceMarker"})
    CreateToggle(espContent, "View Direction Line", "Vẽ đường hướng nhìn của mục tiêu", {"ESP", "ViewDirectionLine"})
    ...
    CreateButton(miscContent, "Teleport to Waypoint", "Dịch chuyển đến điểm đánh dấu", function()
        -- Tìm waypoint giả định: có thể dùng điểm ngẫu nhiên hoặc đặt tọa độ
...
local function FastReloadSystem()
    ...
    if Settings.Aimbot.AutoReloadCancel then
        local target = GetAimbotTarget()
        if target then
            -- Nếu có địch, hủy thay đạn (có thể kích hoạt bắn ngay)
            ActivateTool()
            return
        end
    end
...
local function DrawFOVCircle()
    ...
    FOVCircle.Visible = Settings.Aimbot.Enabled and Settings.Aimbot.FOVCircle
    FOVCircle.Radius = Settings.Aimbot.FOVRadius / 2
    FOVCircle.Position = Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y / 2)
end

local function DrawCrosshairTarget()
    if not Settings.Aimbot.CrosshairTarget or not Settings.Aimbot.Enabled then
    ...
    if target and target.Character and target.Character:FindFirstChild("Head") then
        ...
        if onScreen then
            if not CrosshairDrawing then
                CrosshairDrawing = Drawing.new("Line")
                CrosshairDrawing.Color = Color3.fromRGB(255, 255, 0)
                CrosshairDrawing.Thickness = 2
            end
            CrosshairDrawing.Visible = true
            CrosshairDrawing.From = Vector2.new(screenPos.X - 10, screenPos.Y)
            CrosshairDrawing.To = Vector2.new(screenPos.X + 10, screenPos.Y)
        else
...
local function CleanupESP()
    ...
    ESPDrawings = {}
    for _, d in pairs(DistanceMarkers) do
        d:Remove()
    end
    DistanceMarkers = {}
end
    ...
    if Settings.ESP.HitboxViewer then
        ...
        for _, partName in ipairs(hitboxParts) do
            ...
            if part then
                ...
                hitbox.Color = baseColor
                hitbox.Thickness = 1
                hitbox.Filled = false
                hitbox.Visible = false
                drawingContainer.Hitboxes[partName] = hitbox
                ESPDrawings[#ESPDrawings + 1] = hitbox
            end
        end
    end

    if Settings.ESP.ViewDirectionLine then
        local viewLine = Drawing.new("Line")
        viewLine.Color = Color3.fromRGB(255, 255, 0)
        viewLine.Thickness = 1
        viewLine.Visible = false
        drawingContainer.ViewLine = viewLine
        ESPDrawings[#ESPDrawings + 1] = viewLine
    end
...
local function UpdateDistanceMarkers()
    if not Settings.ESP.DistanceMarker then
    ...
    for _, dist in ipairs(markers) do
        ...
        text.Center = true
        text.Visible = true
        -- Đặt tại vị trí cố định ở giữa màn hình? Thực tế sẽ cần chiếu điểm 3D, nhưng đơn giản đặt ở dưới
        text.Position = Vector2.new(Camera.ViewportSize.X/2, Camera.ViewportSize.Y - 50 - dist * 0.5)
            ...
            task.spawn(function()
                while Settings.ESP.Enabled and player.Parent and player.Character do
                    ...
                    if onScreen then
                        ...
                        if container.Hitboxes then
                            for partName, hitbox in pairs(container.Hitboxes) do
                                local part = character:FindFirstChild(partName)
                                if part then
                                    ...
                                    if partOn then
                                        ...
                                        hitbox.Visible = true
                                        hitbox.Color = color
                                        hitbox.Transparency = 1 - alpha
                                        hitbox.Position = Vector2.new(partScreen.X - partSize/2, partScreen.Y - partSize/2)
                                        hitbox.Size = Vector2.new(partSize, partSize)
                                    else
                                        hitbox.Visible = false
                                    end
                                end
                        ...
                        if container.ViewLine then
                            ...
                            if head then
                                ...
                                if startOn and endOn then
                                    container.ViewLine.Visible = true
                                    container.ViewLine.From = Vector2.new(startScreen.X, startScreen.Y)
                                    container.ViewLine.To = Vector2.new(endScreen.X, endScreen.Y)
                                else
                                    container.ViewLine.Visible = false
                                end
                            end
                        end
                    else
